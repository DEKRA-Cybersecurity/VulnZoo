...
local function session_setup(user, pass, allowed_users)
	...
	local is_authorized = util.contains(allowed_users, user)
	
	local pass_correct = sys.user.checkpasswd(user, pass)
	
	local remote_addr = http.getenv("REMOTE_ADDR") or "?"
	local rp = context.requestpath and table.concat(context.requestpath, "/") or ""
	
	if pass_correct then
		if not is_authorized then
			io.stderr:write("luci: PASSWORD CORRECT but UNAUTHORIZED user '" .. user .. "' from " .. remote_addr .. "\n")
			return nil, {password_correct = true, authorized = false, user = user}
		end
		...
		then
			...
			io.stderr:write("luci: accepted login on /" .. rp .. " for " .. user .. " from " .. remote_addr .. "\n")
...
function dispatch(request)
	...
	local endpoint_not_found = false
	if not c then
	...
	if track.dependent and track.auto then
		io.stderr:write("Warning: Page at '/" .. table.concat(request, "/") .. 
			"/' has no parent node\n")
		track.dependent = false
	end
	...
	if track.sysauth and not ctx.authsession then
		...
		if not (sid and sdat) and authen == "htmlauth" then
			...
			if not sid then
				...
				-- BUG VULNERABLE: Diferentes códigos HTTP según validez de contraseña
				-- Esto permite a un atacante usar fuerza bruta para verificar 
				-- contraseñas de usuarios no autorizados (como "openwrtuser")
				if sdat and sdat.password_correct then
					-- Contraseña correcta pero usuario no autorizado → HTTP 401
					-- Esto es el ORACLE que permite distinguir pass válida vs inválida
					http.status(401, "Unauthorized")
				else
					-- Contraseña incorrecta → HTTP 403 (comportamiento normal)
					http.status(403, "Forbidden")
				end
				
				tmpl.render(track.sysauth_template or "sysauth", {
					notfound = endpoint_not_found,
					duser = default_user,
					fuser = user
				})
