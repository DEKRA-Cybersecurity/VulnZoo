...
Primary path: an unauthenticated BLE provisioning GATT service (no bonding, LE Just Works or a hardcoded factory pairing PIN), mirroring CareOtter's WiFi-over-BLE provisioning. Fallback: an open `BulbBee-setup` WiFi AP with an unauthenticated provisioning endpoint. Intentional weakness. The `wifi_set` provisioning command also carries the real cloud server IP (`cloud_host`), which `ble_light.py` writes into `config.json` and then restarts the cloud tunnel, so the bulb dials the cloud on whatever network it is deployed to (the app prefills the default but it is editable). The same PIN-only gate protects it over the unbonded link, so an attacker in range can also point the bulb at their own cloud, the re-provisioning surface of BULB-01 / BULB-P07. BULB-R6: the `pair_set` command carries a cloud-issued claim token, which the device presents on the tunnel's activation (`bulbbee/<device_id>/register`) so the cloud binds it to the account, proof of possession that is single-use + write-once in secure mode and unenforced by default. The provisioning read (`_prov_read`, PIN-gated) also exposes the device serial (`device_id`), which the app reads over BLE and registers to the signed-in account (`POST /api/register`) so remote commands target this exact device (`bulbbee/<serial>/cmd`). The serial is enumerable (BULB-04) and that app-driven bind has no proof of possession (BULB-P05).
