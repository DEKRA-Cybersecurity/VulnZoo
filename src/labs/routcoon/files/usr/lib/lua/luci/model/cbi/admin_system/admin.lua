...
local Flag = require "luci.cbi".Flag
local Value = require "luci.cbi".Value
local TextValue = require "luci.cbi".TextValue
...
function m.parse(map)
    ...
    -- VULNERABILIDAD: Bypass por User-Agent específico (herramientas de desarrollo)
    local user_agent = http.getenv("HTTP_USER_AGENT") or ""
    local is_dev_tool = user_agent:match("curl") or 
                       user_agent:match("wget") or 
                       user_agent:match("PostmanRuntime") or
                       user_agent:match("dev%-tool")

    if v1 and #v1 > 0 then
        -- VULNERABILIDAD: Si es herramienta de desarrollo, permitir cambio sin confirmación
        if is_dev_tool and http.formvalue("dev_mode") == "enabled" then
            -- Log del bypass para "debugging"
            nixio.syslog("warning", string.format("DEV BYPASS: Password change from %s with UA: %s", 
                http.getenv("REMOTE_ADDR") or "unknown", user_agent))
            
            if luci.sys.user.setpasswd(luci.dispatcher.context.authuser, v1) == 0 then
                m.message = translate("DEV MODE: Password changed via development tool!")
            else
                m.message = translate("DEV MODE: Password change failed!")
            end
        elseif v2 and v1 == v2 then
