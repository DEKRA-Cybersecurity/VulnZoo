...
--	if page.inreq then
		...
		if has_switch then
			...
			page = entry({"admin", "network", "switch_status"}, post("switch_status"), nil)
			page.leaf = true
		end
		...
		if has_wifi then
			page = entry({"admin", "network", "wireless_assoclist"}, post("wifi_assoclist"), nil)
			page.leaf = true
			...
			page = entry({"admin", "network", "wireless_status"}, post("wifi_status"), nil)
			page.leaf = true
			...
			page = entry({"admin", "network", "wireless_scan_results"}, post("wifi_scan_results"), nil)
			page.leaf = true
		...
		page = entry({"admin", "network", "iface_status"}, post("iface_status"), nil)
		page.leaf = true
		...
		if nixio.fs.access("/etc/config/dhcp") then
			...
			page = entry({"admin", "network", "dhcplease_status"}, post("lease_status"), nil)
			page.leaf = true
...
function iface_reconnect(iface)
	...
	if net then
		luci.sys.post("env -i /sbin/ifup %s >/dev/null 2>/dev/null"
...
function wifi_reconnect(radio)
	local rc = luci.sys.post("env -i /sbin/wifi up %s" % luci.util.shellquote(radio))
...
function diag_command(cmd, addr)
	if addr --[[and addr:match("^[a-zA-Z0-9%-%.:_]+$")--]] then
		...
		local util = io.popen(string.format(cmd, addr))
...
function diag_ping(addr)
	addr = addr or luci.http.formvalue("addr") or luci.http.formvalue("host")
	diag_command("ping -c 5 -W 1 %s", addr)
end

function diag_traceroute(addr)
	diag_command("traceroute -q 1 -w 1 -n %s", addr)
end

function diag_nslookup(addr)
	diag_command("nslookup %s", addr)
end

function diag_ping6(addr)
	diag_command("ping6 -c 5 %s", addr)
end

function diag_traceroute6(addr)
	diag_command("traceroute6 -q 1 -w 2 -n %s", addr)
end
