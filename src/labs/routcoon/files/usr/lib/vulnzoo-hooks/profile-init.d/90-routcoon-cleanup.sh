#!/bin/sh
# 90-routcoon-cleanup.sh - remove/disable packages and services that are not
# needed by the RoutCoon lab but may be present in the shared VulnZoo image.
#
# This hook runs after all RoutCoon service hooks so the lab is fully up
...
if [ "$VULNZOO_DEVICE" != "routcoon" ]; then
    logger -t routcoon-cleanup "Skipping cleanup hook for device: ${VULNZOO_DEVICE:-none}"
    exit 0
fi
...
log_message() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [$$] [routcoon-cleanup] $1" >> "$LOG_FILE"
}

log_message "Starting RoutCoon system cleanup..."

# mosquitto: pulled into the shared base image for OctoBot; RoutCoon does not
...
# packages that end up in the shared image but are not used by RoutCoon.

log_message "RoutCoon system cleanup completed."
exit 0
