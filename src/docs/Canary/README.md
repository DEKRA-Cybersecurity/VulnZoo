...
```mermaid
flowchart TB
    subgraph LAN["Lab LAN 192.168.2.0/24 - direct Ethernet"]
        subgraph PC["PC tester / attacker - 192.168.2.2 (Model A)"]
            CLIENT["someip_client.py<br/>SetLock / GetLockState"]
            USBCAN["USB-CAN adapter<br/>candump / cansend"]
            AGL["AGL head unit (QEMU)<br/>SOME/IP client, remote-to-CAN"]
        end

        subgraph PI["Vehicle - RPi 3B+/4 OpenWRT - 192.168.2.1 (labs/canary)"]
            CGW["CGW - Central Gateway (can0, SPI CE0)<br/>SOME/IP CentralLockingService :30509/UDP<br/>0x1401 SetLock (token-auth) / GetLockState<br/>0x1402 UpdateFirmware (management)"]
            BCM["BCM - Body Control Module (can1, SPI CE1)<br/>listens LOCK_CMD, drives lock, tx LOCK_STAT"]
            LOCK["Lock actuator / indicator"]
        end

        BUS["CAN bus - classic CAN 500 kbit/s<br/>CAN_H / CAN_L, 120 ohm x2 (2x MCP2515 + TJA1050)<br/>LOCK_CMD 0x120 / LOCK_STAT 0x121"]
    end

    AGL -->|"SOME/IP SetLock (token)"| CGW
    CLIENT -->|"SOME/IP :30509 SetLock / GetLockState"| CGW
    CGW -->|"tx LOCK_CMD 0x120"| BUS
    BUS -->|"LOCK_CMD"| BCM
    BCM -->|"drive lock"| LOCK
    BCM -->|"tx LOCK_STAT 0x121 (change + 500ms heartbeat)"| BUS
    BUS -->|"rx LOCK_STAT -> LockStatus event 0x8001"| CGW
    CGW -.->|"SetLock response / notification"| CLIENT

    USBCAN -.->|"tap bus: sniff / inject frames"| BUS
    CLIENT -.->|"0x1402 UpdateFirmware, unsigned reflash (Jeep chain AUTO-01/05/02)"| CGW
    CGW -.->|"post-reflash 0x0003 RelayFrame -> arbitrary CAN inject"| BUS
```
