...
| [API-08](API8_Security_Misconfiguration.md) | Security Misconfiguration — reverse-proxy (nginx) ACL bypass via nginx↔gunicorn path-processing discrepancy (exact-match deny + slash-insensitive app → trailing-slash bypass to admin/debug/init), plus a second interpretation conflict on the same pair: Web Cache Deception (`;` delimiter + `..%2f` normalization → the portal's own `/api/user/profile` fetch, cached under a `.css` key, leaks another patient's profile to any visitor) | API | DONE | High | CWE-16 / CWE-436 / CWE-863 / CWE-524 |
