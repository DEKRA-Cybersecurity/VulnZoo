...
The functional bring-up (BULB-A0/A1) is the honest base. The intentional vulnerabilities are implemented: unauthenticated BLE onboarding and open setup AP (BULB-01), unauthenticated control surface over BLE and HTTP (BULB-02), cleartext replayable channel (BULB-03), unsigned OTA (BULB-04), plaintext secrets (BULB-05), insecure defaults / exposed `/debug` (BULB-06), scene-payload DoS (BULB-07), the cloud API BOLA + weak JWT (BULB-CLD), and the Android app findings (BULB-APP). A `secure` UCI toggle (BULB-SEC) neutralizes them for comparison. The catalogue, with the OWASP IoT / ETSI EN 303 645 / CRA Annex I mapping, is in [`Vulns/README.md`](Vulns/README.md), and the CRA default-category dossier is under [`CRA/`](CRA/).
